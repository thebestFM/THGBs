...
class RecurrentRGCN(nn.Module):
    def __init__(self, decoder_name, encoder_name, num_ents, num_rels, num_static_rels, num_words, num_times, time_interval, h_dim, opn, history_rate, sequence_len, num_bases=-1, num_basis=-1,
        ...
        self.num_rel_slots = self.num_rels * (2 if self.add_inverse else 1)
        ...
        self.emb_rel = torch.nn.Parameter(torch.Tensor(self.num_rel_slots, self.h_dim), requires_grad=True).float()
        ...
        if self.use_static:
            ...
            num_static_slots = self.num_static_rels * (2 if self.add_inverse else 1)
            self.statci_rgcn_layer = RGCNBlockLayer(self.h_dim, self.h_dim, num_static_slots, num_bases,
        ...
        if decoder_name == "timeconvtranse":
            ...
            self.rdecoder_re1 = TimeConvTransR(self.num_rel_slots, h_dim, input_dropout, hidden_dropout, feat_dropout)
            self.rdecoder_re2 = TimeConvTransR(self.num_rel_slots, h_dim, input_dropout, hidden_dropout, feat_dropout)
        else:
            raise NotImplementedError 
    ...
    def forward(self, g_list, static_graph, use_cuda):
        ...
        for i, g in enumerate(g_list):
            ...
            x_input = torch.zeros(self.num_rel_slots, self.h_dim).float().cuda() if use_cuda else torch.zeros(self.num_rel_slots, self.h_dim).float()
    ...
    def rel_raw_mode(self, pre_emb, r_emb, time_embs, all_triples):
        scores_re = self.rdecoder_re1.forward(pre_emb, r_emb, time_embs, all_triples).view(-1, self.num_rel_slots)
