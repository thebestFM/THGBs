...
def r2e(triplets, num_rels, add_inverse=True):
...
def build_sub_graph(num_nodes, num_rels, triples, use_cuda, gpu, add_inverse=True):
    def comp_deg_norm(g):
    ...
    uniq_r, r_len, r_to_e = r2e(triples, num_rels, add_inverse=add_inverse)
    g.uniq_r = uniq_r
    g.r_to_e = r_to_e
    g.r_len = r_len
    if use_cuda:
